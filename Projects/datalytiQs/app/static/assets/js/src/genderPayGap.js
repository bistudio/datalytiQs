...
const gender_pay_gap_main = async () => {
  ...
  // append pay gap data to y axis

      const payGap = canvas.selectAll("text").append("g");

      payGap.data(filteredData)
      .enter()
      .append("text")
      .attr("x", 0)
      .attr('y', d => yScale(d.jobCategories) + 9)
      .attr("font-size", "10px")
      .attr("font-style", "bold")
      .attr("fill", "silver")
      .text(d => `${"Pay Gap: "+ "£" + d['medianPayGap']}` +" per hr "+ ", (" + `${Math.round(d['medianPayGapPct'] * 100,2) + "%)"}`)
      ;
...
const createAxes = (data) => {
    ...
    // appliy animation to the malePerson
        ...
        .append("title")
        .attr("class", "tooltip")
        .text(d => `${d['jobCategories']+ ' (M)'} - £${d['maleMedianHourlyRate']}`);
...
// append title to femalePerson
        ...
        .append("title")
        .attr("class", "tooltip")
        .text(d => `${d['jobCategories'] + ' (F)'} - £${d['femaleMedianHourlyRate']}`);
